...
// I2C pins for the AHT20 / BMP280 / LTR390 sensors.
// This board's default I2C pins are GPIO3 (SDA) / GPIO4 (SCL),
// but the sensors here are wired to GPIO5 (SDA) / GPIO6 (SCL),
// so we set that explicitly instead of relying on the default.
#define SENSOR_SDA_PIN 5
#define SENSOR_SCL_PIN 6
...
void setup() {
 ...
 // Use the pins the sensors are actually wired to (GPIO5 = SDA,
 // GPIO6 = SCL) instead of this board's default I2C pins
 // (GPIO3/GPIO4). Without this, Wire.begin() would listen on the
 // wrong pins and all three sensors would fail to initialize.
